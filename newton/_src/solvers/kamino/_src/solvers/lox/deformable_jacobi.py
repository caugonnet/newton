...
class DeformableJacobi:
    """Apply a preallocated scalar or 3-by-3 block Jacobi preconditioner."""
    ...
    def __init__(
        ...
        Args:
            ...
            block_diagonal: Whether to retain each full 3-by-3 diagonal block.
        """
        ...
        if not isinstance(block_diagonal, bool):
            raise ValueError(f"LOX cloth Jacobi block_diagonal must be a bool, got {block_diagonal!r}.")
        ...
        inverse_dtype = wp.mat33 if block_diagonal else wp.float32
        self.inverse_diagonal = wp.empty(self.row_count, dtype=inverse_dtype, device=self.device)
    ...
    def factorize(self) -> None:
        ...
        factorization_kernel = _factorize_block_jacobi if self.block_diagonal else _factorize_jacobi
        wp.launch(
    ...
    def _matvec(
        ...
        apply_kernel = _apply_block_jacobi if self.block_diagonal else _apply_jacobi
        wp.launch(
            apply_kernel,
            dim=self.row_count,
            inputs=[
