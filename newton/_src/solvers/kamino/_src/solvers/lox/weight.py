...
@wp.struct
class BodyWeightAnisotropicResult:
    """Result of one anisotropic inertia-relative weight computation."""

    weight: mat66f
    """Full anisotropic body weight."""
    inverse_weight: mat66f
    """Inverse of the full anisotropic body weight."""
    eta: wp.float32
    """Regularized smooth mass-normalized spectral floor."""
    eigenvalue_min: wp.float32
    """Smallest clamped generalized eigenvalue."""
    eigenvalue_max: wp.float32
    """Largest clamped generalized eigenvalue."""
    status: wp.int32
    """One of the ``BODY_WEIGHT_STATUS_*`` values."""


@wp.struct
class _SymmetricEigenResult:
    vectors: mat66f
    values: vec6f
...
@wp.func
def _compute_symmetric_eigendecomposition_fixed(matrix: mat66f) -> _SymmetricEigenResult:
    """Diagonalize a symmetric matrix with twelve cyclic Jacobi sweeps."""
    diagonalized = matrix
    vectors = mat66f(0.0)
            ...
            while second < 6:
                ...
                if off_diagonal != 0.0:
                    ...
                    while other < 6:
                        ...
                        vector_first = vectors[other, first]
                        vector_second = vectors[other, second]
                        vectors[other, first] = cosine * vector_first - sine * vector_second
                        vectors[other, second] = sine * vector_first + cosine * vector_second
                        other += 1
                second += 1
            first += 1
        sweep += 1

    result = _SymmetricEigenResult()
...
@wp.func
def _make_invalid_anisotropic_body_weight_result() -> BodyWeightAnisotropicResult:
    result = BodyWeightAnisotropicResult()
    result.weight = mat66f(0.0)
    result.inverse_weight = mat66f(0.0)
    result.eta = 0.0
    result.eigenvalue_min = 0.0
    result.eigenvalue_max = 0.0
    result.status = BODY_WEIGHT_STATUS_INVALID
    return result


@wp.func
def compute_body_weight_anisotropic(
    ...
    """Clamp a smooth primal diagonal block relative to spatial inertia.

    Args:
        smooth_diagonal: Smooth body diagonal block ``H_bb``.
        ...
        symmetry_tolerance: Relative symmetry validation tolerance.

    Returns:
        The full weight, inverse, clamp diagnostics, and status.
    """
    if (
        ...
        return _make_invalid_anisotropic_body_weight_result()
    ...
    for index in range(3):
        if not wp.isfinite(inertia_eigenvalues[index]):
            return _make_invalid_anisotropic_body_weight_result()
        for row in range(3):
            if not wp.isfinite(inertia_axes[row, index]):
                return _make_invalid_anisotropic_body_weight_result()
        if inertia_eigenvalues[index] < inertia_floor:
            inertia_eigenvalues[index] = inertia_floor
            status = BODY_WEIGHT_STATUS_REGULARIZED

    sqrt_inertia = wp.mat33f(0.0)
    inverse_sqrt_inertia = wp.mat33f(0.0)
    for row in range(3):
        for col in range(3):
            for index in range(3):
                axis_product = inertia_axes[row, index] * inertia_axes[col, index]
                root = wp.sqrt(inertia_eigenvalues[index])
                sqrt_inertia[row, col] += axis_product * root
                inverse_sqrt_inertia[row, col] += axis_product / root

    sqrt_spatial_mass = mat66f(0.0)
    inverse_sqrt_spatial_mass = mat66f(0.0)
    for index in range(3):
        sqrt_spatial_mass[index, index] = wp.sqrt(regularized_mass)
        inverse_sqrt_spatial_mass[index, index] = 1.0 / wp.sqrt(regularized_mass)
    for row in range(3):
        for col in range(3):
            sqrt_spatial_mass[row + 3, col + 3] = sqrt_inertia[row, col]
    ...
    eta = _compute_symmetric_eigenvalue_min_fixed(_symmetrize_mat66(normalized_smooth))
    ...
    eigen = _compute_symmetric_eigendecomposition_fixed(_symmetrize_mat66(normalized_smooth))
    normalized_weight = mat66f(0.0)
    normalized_inverse = mat66f(0.0)
    eigenvalue_min = wp.float32(beta)
    eigenvalue_max = wp.float32(0.0)
    for index in range(6):
        value = eigen.values[index]
        if not wp.isfinite(value):
            return _make_invalid_anisotropic_body_weight_result()
        regularized_value = wp.max(eta_floor, value)
        clamped = wp.max(sigma * regularized_value, wp.min(beta, regularized_value))
        if clamped != value or regularized_value != value:
            status = BODY_WEIGHT_STATUS_REGULARIZED
        eigenvalue_min = wp.min(eigenvalue_min, clamped)
        eigenvalue_max = wp.max(eigenvalue_max, clamped)
        for row in range(6):
            for col in range(6):
                axis_product = eigen.vectors[row, index] * eigen.vectors[col, index]
                normalized_weight[row, col] += axis_product * clamped
                normalized_inverse[row, col] += axis_product / clamped

    result = BodyWeightAnisotropicResult()
    result.weight = sqrt_spatial_mass @ normalized_weight @ sqrt_spatial_mass
    result.inverse_weight = inverse_sqrt_spatial_mass @ normalized_inverse @ inverse_sqrt_spatial_mass
    result.eta = eta
    result.eigenvalue_min = eigenvalue_min
    result.eigenvalue_max = eigenvalue_max
