...
@wp.kernel
def _increment_factorization_count(factorization_count: wp.array[wp.int32]):
    factorization_count[0] += 1
...
class DeformableBlockLLT:
    ...
    def __init__(
        ...
        if packed_component.shape != (system_matrix.nrow,) or packed_component.dtype != wp.int32:
            raise ValueError("LOX deformable block LLT requires one int32 component per particle.")
        if packed_iterative.shape != (system_matrix.nrow,) or packed_iterative.dtype != wp.int32:
        ...
        self.packed_component = packed_component
        self.packed_world = packed_world
        self.world_active = world_active

        packed_component_np = packed_component.numpy().astype(np.int32, copy=False)
        packed_iterative_np = packed_iterative.numpy().astype(np.int32, copy=False)
        ...
        self.operator = DenseLinearOperatorData(info=self.info, mat=self.matrix)
    ...
    def factorize(self) -> None:
        ...
        wp.launch(
            _increment_factorization_count,
            dim=1,
            outputs=[self.factorization_count],
            device=self.device,
        )
        wp.launch(
