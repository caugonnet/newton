...
from .deformable_linear import DeformableCRState
...
def _mapped_particle_worlds(model: Model, particle_world: np.ndarray) -> np.ndarray:
    world_count = int(model.world_count)
    if world_count < 1:
        raise ValueError("LOX deformables require at least one Newton world.")
    if np.any(particle_world < -1) or np.any(particle_world >= world_count):
    ...
    if np.any(particle_world == -1):
        ...
        particle_world = particle_world.copy()
        particle_world[particle_world == -1] = 0
    return particle_world.astype(np.int32, copy=False)


def _coloring_covers_elements(
    colors: np.ndarray,
    triangle_indices: np.ndarray,
    bending_indices: np.ndarray,
    tetrahedron_indices: np.ndarray,
) -> bool:
    if colors.ndim != 1 or np.any(colors < 0):
        return False
    for elements in (triangle_indices, bending_indices, tetrahedron_indices):
        for vertices in elements:
            ...
            if np.unique(colors[unique_vertices]).size != unique_vertices.size:
                return False
    return True


def _color_deformable_topology(
    particle_count: int,
    triangle_indices: np.ndarray,
    bending_indices: np.ndarray,
    tetrahedron_indices: np.ndarray,
    ...
    for elements in (triangle_indices, bending_indices, tetrahedron_indices):
        for vertices in elements:
            unique_vertices = np.unique(vertices)
            for first_index, first in enumerate(unique_vertices):
                for second in unique_vertices[first_index + 1 :]:
                    adjacency[int(first)].add(int(second))
                    adjacency[int(second)].add(int(first))

    colors = np.full(particle_count, -1, dtype=np.int32)
    order = sorted(range(particle_count), key=lambda particle: (-len(adjacency[particle]), particle))
    for particle in order:
        neighbor_colors = {int(colors[neighbor]) for neighbor in adjacency[particle] if colors[neighbor] >= 0}
        color = 0
...
def _deformable_particle_colors(
    model: Model,
    triangle_indices: np.ndarray,
    bending_indices: np.ndarray,
    tetrahedron_indices: np.ndarray,
) -> tuple[np.ndarray, str]:
    model_colors = getattr(model, "particle_colors", None)
    if model_colors is not None and tuple(model_colors.shape) == (model.particle_count,):
        colors = model_colors.numpy().astype(np.int32, copy=False)
        if _coloring_covers_elements(colors, triangle_indices, bending_indices, tetrahedron_indices):
            _, colors = np.unique(colors, return_inverse=True)
            return colors.astype(np.int32, copy=False), "model"

    return (
        _color_deformable_topology(
...
def _deformable_particle_components(
    ...
    """Return deterministic structural-component labels and world counts."""
    ...
    particle_component = np.asarray([root_to_component[int(root)] for root in roots], dtype=np.int32)
    component_world = particle_world[np.asarray(unique_roots, dtype=np.int32)].astype(np.int32, copy=False)
    world_component_counts = np.bincount(component_world, minlength=world_count).astype(np.int32)
    return particle_component, component_world, world_component_counts
...
def validate_deformable_model(model: Model) -> None:
    ...
    particle_world = _require_array(model, "particle_world", (particle_count,))
    mapped_world = _mapped_particle_worlds(model, particle_world)
    ...
    if triangle_count > 0:
        ...
        triangle_world = mapped_world[triangle_indices]
    ...
    if tetrahedron_count > 0:
        ...
        tetrahedron_world = mapped_world[tetrahedron_indices]
    ...
    if edge_count > 0:
        ...
        for edge_vertices in edge_indices:
            ...
            if referenced_vertices.size > 0 and np.any(
                mapped_world[referenced_vertices] != mapped_world[referenced_vertices[0]]
            ):
...
validate_deformable_cloth_model = validate_deformable_model
...
class DeformableTopology:
    ...
    packed_color: wp.array[wp.int32]
    world_particle_offsets: wp.array[wp.int32]
    world_dof_offsets: wp.array[wp.int32]
...
class DeformableFEMSystem:
    ...
    def __init__(
        ...
        preconditioner: str = "incomplete_ldlt",
        preconditioner_regularization: float = 1.0e-6,
        preconditioner_fill_level: int = 0,
        ...
        Args:
            ...
            preconditioner_fill_level: Symbolic incomplete-factor fill level.
            ...
            proximal_relaxation: Relaxation factor for the local coordinate and multiplier update.
        """
        ...
        self.cr_iterations = cr_iterations
        self.weight_sigma = float(weight_sigma)
        self.weight_beta = float(weight_beta)
        self.preconditioner_type = preconditioner
        self.direct_max_particles = direct_max_particles
        self.proximal_iterations = proximal_iterations
        self.proximal_relaxation = float(proximal_relaxation)

        particle_world = _mapped_particle_worlds(model, model.particle_world.numpy())
        triangle_indices_np = (
        ...
        particle_color, self.coloring_source = _deformable_particle_colors(
            model,
            triangle_indices_np,
            bending_indices_np,
            tetrahedron_indices_np,
        )
        self.color_count = int(np.max(particle_color)) + 1
        particle_component, component_world_np, world_component_counts_np = _deformable_particle_components(
            self.particle_count,
        ...
        self.component_count = int(component_world_np.shape[0])
        self.component_world_host = tuple(int(world) for world in component_world_np)
        self.world_component_counts = tuple(int(count) for count in world_component_counts_np)
        component_particle_counts_np = np.bincount(particle_component, minlength=self.component_count).astype(np.int32)
        component_direct_np = component_particle_counts_np <= direct_max_particles
        if direct_max_particles == 0:
            component_direct_np[:] = False
        self.component_particle_counts = tuple(int(count) for count in component_particle_counts_np)
        self.component_direct_host = tuple(bool(direct) for direct in component_direct_np)
        self.has_direct_components = bool(np.any(component_direct_np))
        ...
        packed_color_np = particle_color[packed_to_newton_np]
        self.particle_color_groups = tuple(
            wp.array(
                np.flatnonzero(packed_color_np == color).astype(np.int32),
                dtype=wp.int32,
                device=self.device,
            )
        ...
        world_counts = np.bincount(packed_world_np, minlength=model.world_count)
        world_particle_offsets_np = np.concatenate(
            (np.zeros(1, dtype=np.int32), np.cumsum(world_counts, dtype=np.int32))
        )
        world_dof_offsets_np = 3 * world_particle_offsets_np
        ...
        self.topology = DeformableTopology(
            ...
            packed_world=wp.array(packed_world_np, dtype=wp.int32, device=self.device),
            packed_component=wp.array(packed_component_np, dtype=wp.int32, device=self.device),
            packed_color=wp.array(packed_color_np, dtype=wp.int32, device=self.device),
            world_particle_offsets=wp.array(world_particle_offsets_np, dtype=wp.int32, device=self.device),
            world_dof_offsets=wp.array(world_dof_offsets_np, dtype=wp.int32, device=self.device),
        ...
        self.assembly_count = wp.zeros(1, dtype=wp.int32, device=self.device)
        self.cr_solve_count = wp.zeros(1, dtype=wp.int32, device=self.device)
        self.cr_iteration_count = wp.zeros(1, dtype=wp.int32, device=self.device)
        self.direct_solve_count = wp.zeros(1, dtype=wp.int32, device=self.device)
        self.direct_iterations = wp.ones(1, dtype=wp.int32, device=self.device)
        self.direct_residual = wp.zeros(1, dtype=wp.float32, device=self.device)
        self.cr_state = (
            None
            if not self.has_iterative_components
            else DeformableCRState(
                self.system_operator,
                self.candidate_rhs,
                self.smooth_velocity,
                tol=0.0,
                atol=0.0,
                maxiter=self.cr_iterations,
    ...
    def reassemble(
        ...
        """Reassemble around a packed candidate velocity from the same step.

        Args:
            linearization_velocity: Candidate particle velocities [m/s].
            time_step: Per-world simulation time steps [s].

        Raises:
            ValueError: If the time step or velocity array is invalid.
        """
        validate_world_time_step(time_step, self.model.world_count, self.device)
        if linearization_velocity.shape != (self.particle_count,) or linearization_velocity.dtype != wp.vec3:
            raise ValueError("LOX deformable linearization velocity must contain one vec3 per packed particle.")
        if linearization_velocity.device != self.device:
            raise ValueError(
                f"LOX deformable expected linearization velocity on {self.device}, found {linearization_velocity.device}."
            )
        self._assemble_linearization(linearization_velocity, time_step)
    ...
    def solve_candidate(
        ...
        Returns:
            Device-side CR iteration, residual, and tolerance arrays.
        """
        ...
        if self.direct_solver is not None:
            wp.launch(
                ...
                dim=1,
                inputs=[1],
                outputs=[self.direct_solve_count],
                device=self.device,
            )
        ...
        if not self.has_iterative_components:
            return self.direct_iterations, self.direct_residual, self.direct_residual

        wp.launch(
            _increment_counter,
            dim=1,
            inputs=[1],
            outputs=[self.cr_solve_count],
            device=self.device,
        )
        wp.launch(
            _increment_counter,
            dim=1,
            inputs=[self.cr_iterations],
            outputs=[self.cr_iteration_count],
            device=self.device,
        )
        assert self.cr_state is not None
        return self.cr_state(M=self.preconditioner.linear_operator)


DeformableClothTopology = DeformableTopology
DeformableClothSystem = DeformableFEMSystem
