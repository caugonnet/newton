...
class DeformableIncompleteLDLT:
    ...
    def __init__(
        ...
        fill_level: int = 0,
        persistent_row_limit: int = _PERSISTENT_ROW_LIMIT,
        row_active: wp.array[wp.int32] | None = None,
    ):
        """Construct symbolic IC(k) data from an immutable BSR topology.
        ...
        if not isinstance(fill_level, int) or isinstance(fill_level, bool) or fill_level < 0:
            raise ValueError(f"LOX cloth incomplete LDLT fill level must be a non-negative integer, got {fill_level}.")
        if (
            not isinstance(persistent_row_limit, int)
            or isinstance(persistent_row_limit, bool)
            or persistent_row_limit < 0
        ):
            raise ValueError(
                f"LOX cloth persistent ILDLT row limit must be a non-negative integer, got {persistent_row_limit}."
            )
        ...
        self.persistent_row_limit = persistent_row_limit
        ...
        row_fill_levels: list[dict[int, int]] = [{} for _ in range(self.row_count)]
        higher_neighbors: list[dict[int, int]] = [{} for _ in range(self.row_count)]
        for row, column in original_lower_coordinates:
            row_fill_levels[row][column] = 0
            higher_neighbors[column][row] = 0
        for eliminated in range(self.row_count):
            neighbors = sorted(higher_neighbors[eliminated].items())
            for first_index, (first, first_level) in enumerate(neighbors):
                for second, second_level in neighbors[:first_index]:
                    row = max(first, second)
                    column = min(first, second)
                    candidate_level = first_level + second_level + 1
                    current_level = row_fill_levels[row].get(column)
                    if candidate_level <= fill_level and (current_level is None or candidate_level < current_level):
                        row_fill_levels[row][column] = candidate_level
                        higher_neighbors[column][row] = candidate_level

        lower_coordinates = [
            (row, column) for row, row_levels in enumerate(row_fill_levels) for column in sorted(row_levels)
        ]
        ...
        lower_fill_levels_np = np.empty(lower_slot_count, dtype=np.int32)
        for row in range(self.row_count):
            for slot in range(int(lower_offsets_np[row]), int(lower_offsets_np[row + 1])):
                ...
                lower_fill_levels_np[slot] = row_fill_levels[row][column]
        ...
        self.lower_fill_levels = wp.array(lower_fill_levels_np, dtype=wp.int32, device=self.device)
