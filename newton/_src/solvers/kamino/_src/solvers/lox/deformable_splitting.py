...
def _store_dual_impulse(
    ...
    particle = wp.tid()
    dual_impulse[particle] = weight[particle] * dual[particle]
...
@wp.kernel
def _accept_outer_worlds(
    world_accepted: wp.array[wp.bool],
    outer_accepted: wp.array[wp.bool],
):
    world = wp.tid()
    outer_accepted[world] = outer_accepted[world] or world_accepted[world]
...
class DeformableSplittingState:
    ...
    def __init__(self, cloth_system):
        ...
        offsets = cloth_system.topology.world_particle_offsets.numpy()
        world_has_particles = (offsets[1:] > offsets[:-1]).astype("int32")
        ...
        self.contact_residual = wp.zeros(self.num_worlds, dtype=wp.float32, device=self.device)
        ...
        self._contact_world_status = wp.zeros(self.num_worlds, dtype=wp.int32, device=self.device)
        self._contact_global_status = wp.zeros(1, dtype=wp.int32, device=self.device)
        self._iteration_count_unused = wp.zeros(self.num_worlds, dtype=wp.int32, device=self.device)
    ...
    def reset(self, world_mask: wp.array[wp.bool] | None = None) -> None:
        ...
        wp.launch(
            ...
            inputs=[self.cloth_system.topology.packed_world, world_mask],
            outputs=[
    ...
    def finish_iteration(
        ...
        wp.launch(
            ...
            inputs=[world_active],
            outputs=[
                iteration_count if increment_iteration_count else self._iteration_count_unused,
                self.iteration_failed,
                self.consensus_residual,
                self.iterate_residual,
                self.displacement_residual,
        ...
        if contact_system is None:
            self.contact_residual.zero_()
            contact_world_status = self._contact_world_status
            contact_global_status = self._contact_global_status
            contact_world_status.zero_()
            contact_global_status.zero_()
        else:
            contact_system.compute_contact_residuals(
                self.projected_velocity,
                projected_twist=rigid_projected_twist,
            )
            wp.copy(self.contact_residual, contact_system.world_contact_residual)
    ...
    def store_dual_impulse(self) -> None:
        """Persist the physical nodal dual impulse for the next time step."""
        wp.launch(
            _store_dual_impulse,
            dim=self.particle_count,
            inputs=[self.cloth_system.weight, self.dual],
            outputs=[self.dual_impulse],
            device=self.device,
        )

    def accept_projected(self, world_accepted: wp.array[wp.bool]) -> None:
        """Retain projected velocities accepted by the current outer solve."""
        wp.launch(
            ...
            outputs=[self.accepted_velocity],
            device=self.device,
        )
        wp.launch(
            _accept_outer_worlds,
            dim=self.num_worlds,
            inputs=[world_accepted],
            outputs=[self.outer_accepted],
            device=self.device,
        )
