...
def _pack_joint_constraint_reactions(
    ...
    model_joint_num_dynamic_cts: wp.array[wp.int32],
    model_joint_num_kinematic_cts: wp.array[wp.int32],
    model_joint_dynamic_cts_offset_joint_cts: wp.array[wp.int32],
    model_joint_kinematic_cts_offset_joint_cts: wp.array[wp.int32],
    model_joint_dynamic_cts_offset_total_cts: wp.array[wp.int32],
    model_joint_kinematic_cts_offset_total_cts: wp.array[wp.int32],
    joint_reaction: wp.array[wp.float32],
    ...
    dynamic_source = model_joint_dynamic_cts_offset_joint_cts[joint]
    kinematic_source = model_joint_kinematic_cts_offset_joint_cts[joint]
    ...
    for row in range(dynamic_count):
        constraint_impulse[dynamic_destination + row] = time_step * joint_reaction[dynamic_source + row]
    for row in range(kinematic_count):
        constraint_impulse[kinematic_destination + row] = time_step * joint_reaction[kinematic_source + row]
