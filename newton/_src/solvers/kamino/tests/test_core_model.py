...
class TestModelConversions(unittest.TestCase):
    ...
    def test_cable_conversion_preserves_storage_and_excludes_fk_partitions(self):
        """Preserve cable storage offsets without adding ordinary joint rows."""
        ...
        np.testing.assert_array_equal(
            ...
            [int(JointDoFType.CABLE), int(JointDoFType.REVOLUTE)],
        )
    ...
    def test_cable_compatibility_requires_lox_without_fk_or_generic_dynamics(self):
        """Require the supported LOX cable subset during compatibility validation."""
        builder = ModelBuilder()
        builder.begin_world()
        body = builder.add_link(mass=1.0)
        builder.add_shape_box(body, hx=0.1, hy=0.1, hz=0.1)
        builder.add_joint_cable(parent=-1, child=body)
        builder.end_world()
        model = builder.finalize(device=self.default_device, skip_validation_joints=True)
    ...
    def test_cable_compatibility_rejects_cross_world_endpoints(self):
        """Reject cable endpoints owned by different explicit worlds."""
        builder = ModelBuilder()
        builder.begin_world()
        parent = builder.add_link(mass=1.0)
        ...
        builder.add_shape_box(parent, hx=0.1, hy=0.1, hz=0.1)
        builder.add_shape_box(child, hx=0.1, hy=0.1, hz=0.1)
        builder.add_joint_cable(parent=parent, child=child)
        builder.end_world()
        builder.begin_world()
