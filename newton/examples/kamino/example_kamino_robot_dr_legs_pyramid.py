...
def _compute_observation(
    ...
    for i in range(36):
        observation[world_index, 34 + i] = joint_q[joint_coord_start + i]
...
def _apply_policy_actions(
    actions: wp.array2d[float],
    actuated_dof_indices: wp.array[int],
    joint_target_count_per_world: int,
    action_scale: float,
    joint_target_q: wp.array[float],
):
    world_index, action_index = wp.tid()
    target_index = world_index * joint_target_count_per_world + actuated_dof_indices[action_index]
...
class Example:
    def __init__(self, viewer: newton.viewer.ViewerBase, args):
        ...
        self.actuated_dof_indices = [
            index for index, stiffness in enumerate(world_builder.joint_target_ke) if stiffness > 0.0
        ]
        if len(self.actuated_dof_indices) != ACTION_DIM:
            raise ValueError(
                f"DR Legs policy expects {ACTION_DIM} actuated joints, found {len(self.actuated_dof_indices)}"
            )
        ...
        self.coulomb_solve_statistics = None
        if args.coulomb_solve_statistics:
            forward_solver = self.solver._solver_kamino._solver_fd
            self.coulomb_solve_statistics = forward_solver.enable_coulomb_solve_statistics()

        if self.dynamics_solver == "lox":
            if not args.direct_structural:
                penalty_scales = self.solver.joint_penalty_scale_seed(self.sim_dt)
        ...
        self.actuated_dof_indices_wp = wp.array(
            self.actuated_dof_indices,
            dtype=wp.int32,
            device=self.device,
        )
    ...
    def print_coulomb_solve_statistics(self) -> None:
        """Print the collected local Coulomb-solve iteration distributions."""
        statistics = self.coulomb_solve_statistics
        if statistics is None:
            return

        branch_histogram = statistics.branch_histogram.numpy()
        expansion_histogram = statistics.expansion_histogram.numpy()
        root_histogram = statistics.root_histogram.numpy()
        failure_counts = statistics.failure_counts.numpy()

        def percentile(histogram: np.ndarray, quantile: float) -> int:
            sample_count = int(np.sum(histogram))
            if sample_count == 0:
                return 0
            target = max(1, int(np.ceil(quantile * sample_count)))
            return int(np.searchsorted(np.cumsum(histogram), target))

        def deciles(histogram: np.ndarray) -> str:
            return ",".join(f"p{quantile}={percentile(histogram, quantile / 100.0)}" for quantile in range(10, 101, 10))

        for source, source_name in enumerate(statistics.SOURCE_NAMES):
            branches = branch_histogram[source]
            expansion = expansion_histogram[source]
            root = root_histogram[source]
            sliding_expansion = expansion.copy()
            sliding_expansion[0] = 0
            sliding_root = root.copy()
            sliding_root[0] = 0
            branch_text = ", ".join(
                f"{name}={int(branches[branch])}" for branch, name in enumerate(statistics.BRANCH_NAMES)
            )
            print(f"[COULOMB_STATS] source={source_name} calls={int(np.sum(branches))} {branch_text}")
            print(
                f"[COULOMB_STATS] source={source_name} expansion "
                f"deciles={deciles(sliding_expansion)} "
                f"p95={percentile(sliding_expansion, 0.95)} "
                f"p99={percentile(sliding_expansion, 0.99)} "
                f"max={int(np.flatnonzero(sliding_expansion)[-1]) if np.any(sliding_expansion) else 0}"
            )
            print(
                f"[COULOMB_STATS] source={source_name} root "
                f"deciles={deciles(sliding_root)} "
                f"p95={percentile(sliding_root, 0.95)} "
                f"p99={percentile(sliding_root, 0.99)} "
                f"max={int(np.flatnonzero(sliding_root)[-1]) if np.any(sliding_root) else 0} "
                f"unbracketed={int(failure_counts[source, 0])} "
                f"unconverged={int(failure_counts[source, 1])}"
            )
            expansion_tail = ",".join(
                f"{iteration}:{int(expansion[iteration])}"
                for iteration in range(10, len(expansion))
                if expansion[iteration] > 0
            )
            root_tail = ",".join(
                f"{iteration}:{int(root[iteration])}" for iteration in range(10, len(root)) if root[iteration] > 0
            )
            print(
                f"[COULOMB_STATS] source={source_name} "
                f"expansion_tail={expansion_tail or 'none'} root_tail={root_tail or 'none'}"
            )
    ...
    def create_parser():
        ...
        parser.add_argument(
            ...
            choices=("incomplete_ldlt", "jacobi"),
        ...
        parser.add_argument(
            "--direct-structural",
            action="store_true",
            help="Use the direct Schur structural-joint solve with LOX.",
        )
        parser.add_argument(
            "--coulomb-solve-statistics",
            action="store_true",
            help="Collect local Coulomb bracket and root iteration histograms.",
        )
        return parser
...
if __name__ == "__main__":
    ...
    example.print_coulomb_solve_statistics()
